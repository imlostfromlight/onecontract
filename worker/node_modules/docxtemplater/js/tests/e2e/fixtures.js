...
var fixtures = [{
...
}), _objectSpread(_objectSpread({}, noInternals), {}, {
  ...
  error: function error(_error, _ref) {
    var errorVerifier = _ref.errorVerifier;
    expect(_error.properties.errors.length).to.equal(1);
    expect(_error.properties.errors[0].properties.rootError.message).to.equal("Filter 'x' is not defined");
    errorVerifier(_error, Errors.XTTemplateError, wrapMultiError({
      name: "ScopeParserError",
      message: "Scope parser compilation failed",
      properties: {
        explanation: 'The scope parser for the tag "name | x" failed to compile',
        rootError: {
  ...
  options: {
    parser: expressionParser.configure({
      preCompile: function preCompile(tag, meta) {
        if (tag[0] === "_") {
  ...
  return _objectSpread(_objectSpread({}, noInternals), {}, {
  ...
  options: {
    modules: function modules() {
      return [{
        ...
        optionsTransformer: function optionsTransformer(options, doc) {
          var loopMod = doc.findModule("LoopModule");
    ...
    parser: expressionParser.configure({
      csp: true,
      postCompile: function postCompile(tag, meta, expr) {
  ...
  content: "<w:p><w:r><w:t>zozo{ab</w:t></w:r></w:p><w:p><w:r><w:t>cde}</w:t></w:r></w:p>",
  options: {
    syntax: {
  ...
  content: '<w:r><w:t xml:space="preserve">Hello {first</w:t></w:r><w:r><w:br/></w:r><w:r><w:t xml:space="preserve">name}</w:t></w:r>',
  options: {
    syntax: {
      preserveNewlinesInTags: true
    }
