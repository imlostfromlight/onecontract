...
// .wrangler/tmp/bundle-5Jw50G/strip-cf-connecting-ip-header.js
...
async function router(request, env2) {
  ...
  }
  {
    ...
    if ((p = matchPath("/api/documents/public/:uuid/debug", path)) && method === "GET")
      return R(await debugDocFile(request, env2, p));
    if ((p = matchPath("/api/documents/public/:uuid/b64", path)) && method === "GET")
      return R(await docFileB64(request, env2, p));
...
async function docFileB64(request, env2, params) {
  try {
    const doc = await env2.DB.prepare("SELECT file_data,file_name,file_mime FROM documents WHERE uuid = ?").bind(params.uuid).first();
    if (!doc || !doc.file_data)
      return json({ detail: "File not found" }, 404);
...
__name(docFileB64, "docFileB64");
async function debugDocFile(request, env2, params) {
  try {
    const doc = await env2.DB.prepare("SELECT length(file_data) as b64len, substr(file_data,1,8) as prefix, file_name, file_mime FROM documents WHERE uuid = ?").bind(params.uuid).first();
    if (!doc)
      return json({ detail: "Not found" }, 404);
...
__name(debugDocFile, "debugDocFile");
...
// .wrangler/tmp/bundle-5Jw50G/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-5Jw50G/middleware-loader.entry.ts
